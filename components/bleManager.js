...
const BLE = ({route, navigation}) => {
  ...
  const machine_number = String(number).padStart(3, '0')
  ...
  const handleDiscoverPeripheral = (peripheral) => {
    if(String(peripheral.name).slice(-3) == machine_number){
            ...
            .then((data) => {
              ...
              setTimeout(() => {
                BleManager.write(peripheral.id, service, startMachineCharacteristic, token).then(() => {
                  console.log('Wrote start machine characteristic');
